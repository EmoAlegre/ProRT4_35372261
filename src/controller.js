...
class BiblioController {
    ...
    async add(req, res) {
        ...
        const allowedAttributes = ['nombre', 'autor', 'categoria', 'año', 'ISBN'];
        ...
        if (!libro.nombre || !libro.autor || !libro.categoria || !libro['año'] || !libro.ISBN) {
    ...
    async update(req,res) {
        ...
        // Validar atributos
        if (!libro.id || !libro.nombre || !libro.autor || !libro.categoria || !libro['año'] || !libro.ISBN) {
            return res.status(400).json({ message: 'Faltan atributos requeridos' });
        }

        try {
        const [result] = await pool.query(
            `UPDATE libros SET nombre=(?), autor=(?), categoria=(?), \`año\`=(?), ISBN=(?) WHERE id=(?)`,
            [libro.nombre, libro.autor, libro.categoria, libro['año'], libro.ISBN, libro.id]);
